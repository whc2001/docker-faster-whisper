...
<!-- Please read https://github.com/linuxserver/docker-faster-whisper/blob/gpu/.github/CONTRIBUTING.md -->
...
Simply pulling `lscr.io/linuxserver/faster-whisper:gpu` should retrieve the correct image for your arch, but you can also pull specific arch images via tags.
...
* Image version number:
    ...
    ```bash
    docker inspect -f '{{ index .Config.Labels "build_version" }}' lscr.io/linuxserver/faster-whisper:gpu
    ```
...
* Update the image:
    ...
    ```bash
    docker pull lscr.io/linuxserver/faster-whisper:gpu
    ```
...
docker build \
  --no-cache \
  --pull \
  -t lscr.io/linuxserver/faster-whisper:gpu .
```
